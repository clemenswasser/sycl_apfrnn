...
void run_gpu_neighbor_search(queue &q, const std::vector<sycl::float3> &h_pos,
                             float radius) {
  int N = h_pos.size();
  ...
  std::cout << "1. Morton Codes Gen:   " << timer.stop() / i << " ms\n";
  ...
  std::cout << "2. Hardware Radix Sort:" << timer.stop() / i << " ms\n";
  ...
  std::cout << "3. Position Reorder:   " << timer.stop() / i << " ms\n";
...
#pragma unroll 3
  ...
  std::cout << "4. Neighbor Search:    " << timer.stop() / i << " ms\n";
  std::cout << "------------------------------------------\n";
  std::cout << "TOTAL GPU PIPELINE:    " << total_timer.stop() / i << " ms\n\n";
